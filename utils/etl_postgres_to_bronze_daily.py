...
class ETLDataPostgresToDeltaTable():
    ...
    def write_delta_table(self, df, check_point):
        ...
        print(f"Successfully wrote data to the Delta Table for date {self.current_date} at {self.target_path}")
        LOGGER.info(f"Successfully wrote data to the Delta Table for date {self.current_date} at {self.target_path}")
        self.spark.stop()
